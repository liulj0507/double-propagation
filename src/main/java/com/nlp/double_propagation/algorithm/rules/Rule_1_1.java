...
public class Rule_1_1 implements DoublePropagationRule {

	public static final String RULE_NAME = Rule_1_1.class.getName();
	...
	public List<RuleCheckResult> applyRule(Word word, IntraSentenceWordRelations intraSentenceWordRelations) {
		...
		// de la regla
		List<Word> candidateWords = intraSentenceWordRelations.getWordsGoverningThis(word, REQUIRED_RELATION_TYPE_1);
		...
		candidateWords = intraSentenceWordRelations.getWordsGovernedByThis(word, REQUIRED_RELATION_TYPE_2);
		for (Word candidateWord : candidateWords) {
			if (candidateWord.getPostag() == REQUIRED_TARGETWORD_POSTAG) {
				RuleCheckResult ruleCheckResult = new RuleCheckResult();
				ruleCheckResult.setTriggeringWord(word);
