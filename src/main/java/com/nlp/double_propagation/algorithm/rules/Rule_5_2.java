...
public class Rule_5_2 implements DoublePropagationRule {

	public static final String RULE_NAME = Rule_5_2.class.getName();
	public static final DoublePropagationWordType TRIGGERING_WORD_TYPE = DoublePropagationWordType.TARGET;

	private static final PartOfSpeech REQUIRED_TARGETWORD_POSTAG = PartOfSpeech.OTHER;
	private static final RelationTypes REQUIRED_RELATION_TYPE_1 = RelationTypes.NoPattern;
	...
	public List<RuleCheckResult> applyRule(Word word, IntraSentenceWordRelations intraSentenceWordRelations) {
		...
		for (Word candidateWord : candidateWords) {
			if (candidateWord.getPostag() == REQUIRED_TARGETWORD_POSTAG && candidateWord.getWordForm().equals("no")) {
